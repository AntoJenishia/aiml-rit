...
function StatCountCard({ icon: Icon, label, value, suffix, delayMs = 0 }: {
  icon: LucideIcon; label: string; value: string; suffix?: string; delayMs?: number;
}) {
  ...
  return (
    <div ref={ref} className="premium-card hover-glow group p-6 text-center">
      <div className="mx-auto mb-3 inline-flex rounded-xl bg-blue-50 p-3 text-blue-600 transition-all duration-300 group-hover:bg-blue-600 group-hover:text-white group-hover:shadow-md group-hover:shadow-blue-600/30">
        <Icon className="h-5 w-5" />
      </div>
      <p className="text-4xl font-black text-[#1e3a8a] transition-colors duration-300 group-hover:text-blue-600">
        {count}{suffix ?? ""}
      </p>
      <p className="mt-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</p>
    </div>
  );
}
        ...
        <section className="py-14">
          <div className="mx-auto max-w-7xl px-6">
            <SectionHeading
              ...
              subtitle="Key numbers defining our commitment to excellence."
              align="center"
            />
            <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {statItems.length < 4 && (
                <StatCountCard icon={statItems[0].icon} label="Research Projects" value="24" delayMs={360} />
              )}
            </div>
          </div>
        </section>
                ...
                return (
                  <Link key={item.href} href={item.href}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-slate-800 transition-colors duration-300 group-hover:text-blue-700">{item.label}</p>
