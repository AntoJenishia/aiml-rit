...
function PoGrid() {
  ...
  return (
    <div ref={ref} className="mt-5 grid gap-3 sm:grid-cols-2">
      {aboutData.programmeOutcomes.map((po, i) => (
        <div
          key={po.code}
          className={`premium-card group relative p-4 ${visible ? "reveal-item" : "opacity-0"}`}
          style={{ animationDelay: `${i * 55}ms` }}
        >
          {/* Blue left accent that expands on hover */}
          <div className="absolute inset-y-0 left-0 w-0.5 rounded-l-2xl bg-blue-500 opacity-0 transition-all duration-300 group-hover:w-1 group-hover:opacity-100" />
          <div className="flex items-start gap-3">
            {/* Tiny PO badge — flips blue on hover */}
    ...
    <div className="page-surface">
      <div className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-9">
          ...
          {/* Vision & Mission — clean white cards, blue left border accent */}
          <RevealSection>
