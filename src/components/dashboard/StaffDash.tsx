...
// ── Faculty OD Modal ───────────────────────────────────────────────────────────
function FacultyODModal({onClose,onSuccess}:{onClose:()=>void;onSuccess:()=>void}) {
  ...
  const handleSubmit = async () => {
    ...
    setUploading(true)
    let gpsLocation;
    try { gpsLocation = await fetchLocation() } catch(err:any) { setError(err.message || "GPS location is required."); setUploading(false); return }
    ...
    const res=await fetch("/api/faculty/od",{
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-xl bg-white rounded-xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        <div className="px-5 pt-5 pb-4 border-b border-[#E2E8F0] flex items-center justify-between bg-slate-50">
          <div>
            <h2 className="text-lg font-bold text-slate-800">Apply for Faculty OD</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><label className="block text-xs font-semibold text-slate-700 mb-1.5">Event Name *</label><input value={form.eventName} onChange={e=>set("eventName",e.target.value)} placeholder="e.g. FDP on AI" className={inp}/></div>
...
function StaffDashInner() {
  ...
  const [odSubTab, setOdSubTab] = useState<"requests" | "proofs" | "my-od">("requests")
  const [showAddPortfolio, setShowAddPortfolio] = useState(false)
  const [showFacultyODForm, setShowFacultyODForm] = useState(false)
  ...
  const pendingODs = odRequests.filter(o => o.status === "pending_faculty" || o.status === "post_pending_faculty")
            ...
            {recentActivities.map(activity => (
              <div key={`${activity.type}-${activity.id}`} className="px-5 py-4 flex items-start gap-4 hover:bg-slate-50 transition-colors">
                ...
                <div className="shrink-0 text-right">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${activity.status === 'VERIFIED' || activity.status === 'COMPLETED' || activity.status === 'ACTIVITY_COMPLETED' ? 'bg-green-100 text-green-700' :
  ...
  const renderODRequests = () => {
    // Status can be "pending_faculty" (new flow) or "FACULTY_VERIFICATION" (legacy flow)
    const reqList = odRequests.filter(od => 
      (od.status === "pending_faculty" || od.status === "FACULTY_VERIFICATION") &&
      !od.isFacultyRequest
    )
    const proofList = odRequests.filter(od => od.status === "post_pending_faculty")
    const myOdList = odRequests.filter(od => od.isFacultyRequest)
    const list = odSubTab === "requests" ? reqList : odSubTab === "proofs" ? proofList : myOdList
    ...
    return (
      <div className="bg-white rounded-3xl border border-[#E5E7EB] shadow-sm overflow-hidden p-6">
        ...
        <div className="flex bg-[#F5F6FA] border border-[#E5E7EB] rounded-xl p-1 mb-5">
          ...
          <button onClick={() => setOdSubTab("my-od")}
            className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${odSubTab === "my-od" ? "bg-white shadow-sm text-[#111827]" : "text-[#6B7280]"
              }`}>
            My OD {myOdList.length > 0 && <span className="h-4 w-4 rounded-full bg-indigo-500 text-white text-[9px] font-black flex items-center justify-center">{myOdList.length}</span>}
          </button>
        </div>
        ...
        {loadingOD ? (
          ...
          <div className="space-y-4">
            <div className="flex justify-end">
              <button onClick={() => setShowFacultyODForm(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-[#003087] text-white rounded-lg text-sm font-semibold hover:bg-[#002070] transition-colors shadow-sm">
                <Plus className="h-4 w-4"/> Apply for OD
              </button>
            </div>
            {myOdList.length === 0 ? (
              <div className="flex flex-col items-center py-14 text-center border-2 border-dashed border-[#E5E7EB] rounded-2xl">
                <div className="h-14 w-14 rounded-full bg-indigo-50 flex items-center justify-center mb-3"><FileText className="h-7 w-7 text-indigo-400" /></div>
                <p className="text-sm font-bold text-[#111827]">No personal OD requests</p>
                <p className="text-xs text-[#6B7280] mt-1">Submit your own OD application for events or training you need to attend.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {myOdList.map(od => (
                  <div key={od.id} className="p-4 rounded-2xl border border-[#E5E7EB] bg-white shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex gap-2 mt-2 flex-wrap">
                          {od.signedLetterUrl && <a href={od.signedLetterUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] font-bold text-blue-700 hover:underline"><ExternalLink className="h-3 w-3"/> View Uploaded Proof</a>}
                          {od.finalPdfUrl && <a href={od.finalPdfUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] font-bold text-green-700 hover:underline"><ExternalLink className="h-3 w-3"/> Approved Letter</a>}
                        </div>
                      </div>
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded uppercase tracking-wider shrink-0 ${
                        od.status === "approved" || od.status === "completed" ? "bg-green-100 text-green-700" :
                        od.status === "rejected_hod" ? "bg-red-100 text-red-700" :
                        "bg-amber-100 text-amber-700"
                      }`}>{od.status === "pending_hod" ? "Pending HOD" : od.status === "approved" ? "Approved" : od.status === "completed" ? "Completed" : od.status === "rejected_hod" ? "Rejected" : od.status.replace(/_/g, " ")}</span>
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {showFacultyODForm && (
        <FacultyODModal 
          onClose={() => setShowFacultyODForm(false)} 
          onSuccess={() => { loadData(); alert("Faculty OD application submitted successfully!"); }} 
        />
      )}
